    ...
    <Card className="w-full max-w-lg bg-gradient-card shadow-card hover:shadow-elegant transition-all duration-300 border-0 rounded-3xl overflow-hidden group hover:scale-[1.02]">
      {image && (
        <div className="relative h-48 overflow-hidden">
          ...
          <Badge variant="secondary" className="absolute top-4 right-4 bg-brand-amarillo-relleno text-brand-verde-titulo border border-brand-amarillo-trazo/30 rounded-full px-3 py-1">
            {category}
          </Badge>
        </div>
      )}
      ...
      <div className={`${image ? 'bg-brand-amarillo-relleno' : 'bg-savia-mint'} p-4 relative`}>
        ...
        <div className={image ? 'mt-4' : 'mt-8'}>
          <h3 className="text-xl font-bold text-brand-verde-titulo mb-2 line-clamp-2">{title}</h3>
          <p className="text-brand-gris-savia text-sm line-clamp-3 mb-4">{description}</p>
        </div>
      </div>
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 text-brand-verde-titulo font-medium">
            <Shield className="w-4 h-4" /><span>Trust: {trustScore}%</span>
          </div>
        </div>

        <div className="flex items-center justify-between text-sm">
          <span className="text-brand-gris-savia">Beneficiario:</span>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="text-brand-gris-savia">{raisedAmount.toLocaleString()} XLM</span>
            <span className="text-brand-verde-titulo font-bold">{progressPercentage}%</span>
          </div>
          <Progress value={progressPercentage} className="h-2 bg-brand-amarillo-relleno" />
          <div className="flex items-center justify-between text-sm">
            <span className="text-brand-verde-titulo font-bold">Meta: {goalAmount.toLocaleString()} XLM</span>
            <div className="flex items-center gap-1 text-brand-gris-savia">
              <Clock className="w-4 h-4" /><span>{daysLeft} Días</span>
            </div>
          </div>
        </div>

        <div className="flex gap-3 pt-2">
          <Button variant="donate" className="flex-1 rounded-full" onClick={() => navigate(`/donation-flow/${id}`)}>
            <Heart className="w-4 h-4" /> Donar
          </Button>
          <Button variant="mint" className="flex-1 rounded-full" onClick={() => navigate(`/donation-flow/${id}`)}>
            Ver Detalles
          </Button>
        </div>
      </div>
